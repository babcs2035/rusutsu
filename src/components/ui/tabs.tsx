...
function Tabs({
  ...
  return (
    <TabsPrimitive.Root
      ...
      // base-ui Tabs Root は data-orientation 属性のみを付与するため，
      // data-horizontal / data-vertical 属性セレクタ（Tailwind の data-* バリアント）は
      // 常に非一致（dead class）になる。orientation 別のクラスは付与しない
      // （縦積みが必要な呼び出し元は flex-col を明示する，SUG-9 参照）
  ...
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        ...
        // after 擬似要素の orientation 別位置指定（group-data-horizontal/vertical）は
        // 上記の通り dead class だったため除去（line variant の下線は未使用）
