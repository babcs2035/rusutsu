...
export async function avgTemp(
  ...
  validResorts.forEach(async (name, index) => {
    const resortContainer = container
      .append("div")
      .attr("class", "skiResortData-container")
    ...
    if (shouldCompare) {
      ...
      // console.log("showXAxis", showXAxis);
    } else {
      h3.text("平均気温");
      showXAxis = false;
    }
    ...
    const heatmapContainer = resortContainer
      .append("div")
      .attr("class", `heatmap-container-${name.replace(/\s+/g, "_")}`) //スキー場名の空白を_に置き換え
    ...
    var initialDropdownHeight = 108;

    // console.log("initialDropdownHeight", initialDropdownHeight);
...
function temperatureHeatmap(
  ...
  var mouseover = (_event, d) => {
    d3.selectAll(`[data-id='${d.id}']`)
      ...
      .each(function (d) {
        ...
        let tooltipHtml;
        if (elementClass.includes("snow-week-rect") && !shouldCompare) {
          tooltipHtml = `
          ...
          tooltipHtml = `
                        ...
                        <b>${d.value}℃</b>
                    `;
        } else if (elementClass.includes("snow-depth-rect") && !shouldCompare) {
          tooltipHtml = `
						${formattedWeeks.find(week => week.id === d.id)?.month}月
						${formattedWeeks.find(week => week.id === d.id)?.week}週目 
						<b>${d.value}cm</b>
  ...
  // Read the data
  // d3.json(jsonFile).then(function (jsonData) {
  const skiResortData = forecastsData.find(entry => entry.meta.id === name);
