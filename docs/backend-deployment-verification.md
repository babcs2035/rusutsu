...
## 2026-09-06：旧称・ふりがなの専用テーブル化とレビュー公開

- PostgreSQL 16の使い捨てコンテナで全migrationを適用し、506件の既存スキー場から1,906件のふりがなセグメント・64件の旧称を移行した。
- 元の文字・読み・表示順が全件一致すること、未登録IDに紐付けられないこと、表示順を重複登録できないことを実DBで検証した。専用テーブルにJSON/JSONB列はない。
- 全370テストと型チェックが成功。CLIは通信を模擬し、差分表示、未反映、プレビュー後の変更拒否、2文書同時送信を検証した。
- 既存レビュー20組も形式検証した。17組は通過。naebaは形式不一致、osorakan-snow-parkとrusutsu-resortは対象ID不一致で停止する。元のJSONは変更していない。
- 本番アクセス・公開・DB変更は行っていない。ブラウザーでの実画面操作と、本変更を含む最終Dockerイメージのビルドは未検証。
- 検証用コンテナは作成したIDを指定して削除済み。既存のDB・volumeには触れていない。
