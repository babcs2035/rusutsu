...
クローラーのプログラムが存在するかどうかを示す表示ではありません。

本番の管理画面と、本番APIに接続したローカル管理画面は、同じ参照処理を使います。
DBに採用済みの取得結果があればそれを優先し、なければ本番に同梱した過去の取得JSONを参照します。
そのため「取得結果あり」でも、今日の営業情報を取得したことを意味するわけではありません。
過去のJSONを参照しただけで、DBへ再登録されることもありません。

修正版をデプロイした後、両方の管理画面を再読み込みし、同じスキー場の表示と
対応付け画面のコース・リフト名が一致することを確認してください。
ローカルも同じ修正版を使い、`DATA_API_BASE_URL`を本番URLに設定します。
この修正のためのDB再移行、APIキーの変更、クローラーの再実行は不要です。
