...
- **別の券に追加（チャージ）して使う券は `add_on_to_product_ids` に追加先の券を書く。**
  「トップアップ5時間は、25時間券を購入された方のみチャージ可能」→
  トップアップの product に `add_on_to_product_ids: ["<25時間券のid>"]`、
  `validity` は追加先と同じ `hours_pool`（`hours: 5`）。
  offer は `prior_purchase` ＋ `target_qualification` のまま（単独の代表にしない）。
  requirements の自由文だけで済ませると、画面が組み合わせを計算できない。
  ICカードへの「チャージ購入」（券を買い直してカードに載せるだけ）は追加券ではない
- **`covers_hours_types` にその券が使える営業区分を記録する。**
  ナイター込み1日券は `["regular","night"]`、ナイター券は `["night"]`。
  **資料に記載が無ければ `null`。`false` と決めつけない**
- ★**ナイター営業があるのに、1日券でナイターも滑れるか（ナイターが別料金か）を
  資料で確定できなければ、利用者にチャットで1行で尋ねる。**
  例:「ルスツの1日券でナイターも滑れますか？（公式に記載なし）」
  回答どおり `covers_hours_types` を設定し、`validity.notes_ja` に
  「公式資料に記載はないが、管理者の確認（YYYY-MM-DD）により〜」と残す。
  `null` のままだと画面は「1日券＋ナイター券」で計算する
...
- **早割も `advance_purchase` に含める**（`early_bird` というラベルは無い）。
  ...
  `sales_period` は利用日ではなく照会日（今日）と比べる。販売期間を過ぎた割引は
  照会結果にも画面の料金表にも出ない。販売期間中で期限に間に合い、通常料金より
  安ければ代表になる
...
- **券の分類は `validity` だけで表す**（`product_type` というフィールドは無い）。
  ...
  - 観光用ゴンドラ券・歩行者用乗車券 → **収集対象外**（記録しない）。
    滑走できない券はこのデータの用途外
  - バスツアー・交通付きパック・宿泊込みプラン → **収集対象外**（記録しない）。
    交通費・宿泊費を含む総額はゲレンデで買うリフト券と比べられない。
    宿泊者向けのリフト券割引（リフト券単体の料金）は従来どおり収集する
...
- [ ] 「〇〇券を購入した方のみチャージ可能」「トップアップ」「追加N時間」のように別の券へ足す券に
      `add_on_to_product_ids` があり、追加先の券と `validity.mode` が一致している
