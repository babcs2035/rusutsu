...
- **料金表の列見出しには、`school_levels` を持つ区分の年齢を表示しない。**
  「こども（小学生以下）」「中高生」のように学校区分で決まる区分は、公式表記に
  年齢があっても見出しに「13〜18歳」などを併記しない（年齢は照会の判定にだけ使う）。
  年齢だけで決まる区分（大人・シニア等、`school_levels` が空）は年齢を併記する。
  見出しに年齢を出したいからといって `school_levels` を外したり、年齢を補完したりしない
...
| 利用可能エリアの限定 | `area_ids` |
| 別の券に追加（チャージ）して使う券 | `add_on_to_product_ids` |
...
- **`add_on_to_product_ids`（追加して使う券）**: ★**ルスツの「トップアップ5時間」は
  「25時間券」を購入したICカードにだけチャージできる。** このように単独では買えず、
  別の券に追加して使う券は、追加先の券の product id をここに列挙する
  （`"add_on_to_product_ids": ["prod-25hour"]`）。**空なら単独で買える券。**
  - 追加先の券と同じ `validity.mode` にする（時間を足す券なら両方 `hours_pool`、
    `hours` は追加される時間）
  - offer には従来どおり `discount_reasons: ["prior_purchase"]` と、追加先の券を
    持っている人に限る `target_qualification` を付ける。これで単独の代表料金には選ばれない
  - 画面の計算は、日をまたぐ滑走時間の合計に対して「追加先の券 × M ＋ 追加券 × N」を
    候補にする。追加券は、それが必要になる日までのいずれかの利用日に買う前提で、
    その日の料金（春料金など）を当てる。**この項目が無いと、組み合わせたほうが
    安くても候補に出ない**（実際に 30時間 で25時間券＋トップアップが出ず、
    1日券4日分が表示されていた）
